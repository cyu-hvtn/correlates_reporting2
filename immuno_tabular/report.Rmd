...
numi <- 1
for (d in dats){
cat("\n\n\\newgeometry{top=0.5in, bottom=0.5in, left=.2in, right=.2in}")
base::load(here::here("immuno_tabular", "output", attr(config,"config"), d))
  ...
  for (j in jloop) {
      ...
      if ("collapse_rows" %in% tlf_args){
          collapse_cols <-  tlf_labels[["collapse_rows"]]
          tabjk <- collapse_rows(tabjk, columns = collapse_cols, valign = "top", latex_hline = "none")
