...
getResponder <- function(data,
  ...
  for (i in times){
    for (j in assays){
      ...
      # } else {
      } else if (grepl("nextgen", COR)) {
        data[, paste0(post, "Resp")] <- case_when(
          data[, bl] >= log10(uloqs[j]) ~ NA,
          (data[, bl] < log10(cutoff) & data[, post] >= log10(responderFR*cutoff)) |
  ...
  for (i in v){
    ...
    for (j in subs){
      ...
      retn <- dat %>%
        ...
        summarise(N=n(), Np=sum(!!as.name(i)), Nw=round(sum(!!as.name(weights)),1), rspndr= round(sum(rspndr),1), .groups="drop")
  ...
  for (j in groups){
    ...
    for (i in v){
      ...
      if (nrow(n.j)!=0){
        ...
        if(any(class(ret)=="try-error")) next()
